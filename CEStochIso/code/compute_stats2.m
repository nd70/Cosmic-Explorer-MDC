...
% inputs:
% dir: pointer to directory containing post-processing results
% fileprefixes: cell-array containing each sub-directory with post-processing results (e.g.: {'H1L1/'}, or {'H1L1/','H1V1/','L1V1'}
% outputFileName: filename to write output to
% h0: hubble constant divided by 100 km/s/Mpc
% bias: bias factor for sigma
% notches: array with extra notches to apply
% output:
% a file containing Y(f) and sigma(f)
...
for ii=1:length(fileprefixes)
   ...
   ptEst_f=2/sigma_tot^2 * ptEstInt_f./sensInt_f;
