   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_stochastic.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* stochastic.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* stochastic.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* stochastic (application created by MATLAB Compiler)
		* run_stochastic.sh (shell script that can be run 
	...
	All Platforms:            
		* stochastic.so (shared library, file extension 
		  varies by platform)
		* stochastic.h (library header file)
		* stochastic.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* stochastic.ctf (Component Technology File archive)	
	* stochastic.xml (documentation files)
	* stochastic.pdb (program database file - if 
	  DEBUG option selected)
	* stochastic.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* stochastic.ctf (Component Technology File archive)
	* stochastic_1_0.dll (component containing 
	  compiled M-code)
	...
	* stochastic.jar (Java archive containing Java 
	  interface to M-code in stochastic.ctf)
	* stochastic.ctf (Component Technology File archive,
	  platform-dependent on target machine)
