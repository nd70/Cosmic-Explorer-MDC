function y = windowAndFFT(x, window, fftLength)
%
%  windowAndFFT --- apply window and FFT
% 
%  windowAndFFT(x, window, fftLength) windows, zero-pads (to fftLength)
%  and DFTs a real time series of length N, sampling period deltaT, to
%  produce a complex frequency series of length fftLength/2 + 1 if
%  fftLength is even, or length (fftLength+1)/2 if fftLength is odd
% 
...
% zero-pad x
z = zeros(fftLength-N,1);
xbar = [ x.data.*window ; z ];
...
if ( isfield(x,'fbase') & (~ isnan(x.fbase)) )
  ...
  y_temp = y_temp( 1:floor((fftLength/2 + 1)) );
  y.symmetry = 1;
end
