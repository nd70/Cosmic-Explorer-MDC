function [P, X, Fisher, Sky, plm, P1, P2, h1h2] = plmToPlmPix (sourceFile, lmax, deg)

% arguments:
% 
% lmax           Highest spherical harmonic to use in the search
% deg            Desired resolution for point source map, in degrees
%
...
siderealTime=GPStoGreenwichMeanSiderealTime(timeBins+deltaTime/2);
 ...
 det1=getdetector(detectorPair(1));
 det2=getdetector(detectorPair(2));
 [map,RAs,DECLs,dOmega] = makemap(sourceFile,deg);
 RADECL=[RAs,DECLs];
 h1h2   = Map2CrossPower(det1, det2, map, dOmega, RADECL, siderealTime, numFreqs, flow, deltaF, HAlpha, alpha);
