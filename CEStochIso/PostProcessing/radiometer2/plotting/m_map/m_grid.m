...
xtick=7;
ytick=13;
%xlabels={'-180','-150','-120','-90','-60','-30','0','30','60','90','120','150','180'}';
%xlabels=[0,30,60,90,120,150,180,210,240,270,300,330,360]';
xlabels=[0,60,120,180,240,300,360]';
ylabels=[-90,-75,-60,-45,-30,-15,0,15,30,45,60,75,90]';
...
if ~isempty(xtick),
 ...
 line(reshape([X;NaN+ones(1,m)],(n+1)*m,1),reshape([Y;NaN+ones(1,m)],(n+1)*m,1),...
      'linestyle',glinestyle,'color',gcolor,'linewidth',2,'tag','m_grid_xgrid');
 % Get the tick data
...
if ~isempty(ytick),
 ...
 [n,m]=size(X);
 line(reshape([X;NaN+ones(1,m)],(n+1)*m,1),reshape([Y;NaN+ones(1,m)],(n+1)*m,1),...
      'linestyle',glinestyle,'color',gcolor,'linewidth',2,'tag','m_grid_ygrid');
 % Get the tick data
 [ltx,lty,utx,uty]=maketicks(X,Y,gticklen,gtickdir);
