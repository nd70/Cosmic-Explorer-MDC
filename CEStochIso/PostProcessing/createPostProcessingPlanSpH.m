...
[ignore1, startTimes, ignore2, jobDurations] = ...
  ctextread(jobsFile, '%n %n %n %n', -1, 'commentstyle', 'matlab'); 

% read in params structure from a file
params = readParamsFromFile(paramsFile);
try params.doSphericalHarmonics;     catch error('Not a Spherical Harmonics parameter file (doSphericalHarmonics missing)'); end;
try params.writeNaiveSigmasToFiles;  catch error('writeNaiveSigmasToFiles missing'); end;
if ~params.doSphericalHarmonics
  error('Not a Spherical Harmonics parameter file (doSphericalHarmonics = false)');
end;
