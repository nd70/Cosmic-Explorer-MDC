...
function postProcessScriptFull(paramsFile, jobsFile, outputDir, dSigmaCut, largeSigmaCutoff, ...
                               doRenormalize, modifyFilter, displayResults, applyBadGPSTimes, badGPSTimesFile, printFormats,startTime,stopTime)
...
%  end;
  ...
  %%%%%%
  % ADDED BY AAM: only do pproc between a start and stop tiem
  mask=(startTimes>=startTime & startTimes<=stopTime);
  startTimes=startTimes(mask);
  jobDurations=jobDurations(mask);
  %%%%%%
  numJobs = length(mask);
  numJobs_aftercut=length(startTimes);
  ...
  % Write datestamp
keyboard
% AAM: modified this so the datestamp file goes in current directory
  fid = fopen([ './datestamp.txt' ], 'w');
  fprintf(fid, '%s\n', datestamp);
  fclose(fid);
  ...
  concatCCStats = {};
  for job = 1:numJobs
    if mask(job) % ADDED by AAM: only analyze this job if it is within the start/stop time
	...
	concatNaiveSigmas{job}='null.txt';
	concatCCStats{job}='null.txt';
    end;
  end;
  ...
  % Figures 2-7
% AAM: changed recovery file
  [ptEstimate, errorBar, combinedPtEstInt, combinedSensInt, numSegmentsTotal] = ...
      combineResultsFromMultipleJobs_wss(statsfileprefix, spectrafileprefix, ...
                                     ...
                                     figureLegend, 0, ['./resumeCRfMJwSS.mat' ], printFormats,mask);
